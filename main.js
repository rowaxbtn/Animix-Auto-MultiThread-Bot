...
const mergePetIds = async (headers, proxy) => {
    ...
    while (moms.length > 1) {
        ...
        if ([124, 121, 122, 199, 125].includes(dad)) {
...
async function startMission(user,userCount) {
...
async function main() {
    ...
    while (true) {
        await new Promise( (resolve, reject) => {
            async.eachOfLimit(users,limit_user_per_round, async (user, index) => {
                await startMission(user,index+1);
            }, (err)=> {
                if (err) {
                    log.error("Erro run multi Users: ",err.message);
                    reject(err);
                }
                else resolve()
            })}
